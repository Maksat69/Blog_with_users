...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts, logg_in=current_user.is_authenticated)
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", form=form, logg_in=current_user.is_authenticated)
